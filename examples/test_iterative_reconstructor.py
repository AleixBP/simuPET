...
boxes, img, detections, phi, s = simulator.simulate_simuPET(radioactivity=radioactivity, plot=False, image_path=((840,840),200,None,None))
np.max(detections, axis=1)
boxes, img, *_, phi, s, data_weights = simulator.iterate_simulate_simuPET(nsim=nsim, weights=True, radioactivity=radioactivity, image_path=image_path)
...
sampled_phi = np.linspace(0, np.pi, number_of_phi_samples, endpoint=False) # no need for uniformity
sampled_s = np.linspace(*domain_s, number_of_s_samples) # need uniformity

# Take the chance to define vol too
spacing_of_s = 1. #uniform
...
parallel_sino_data = dens_resample(sino_data, sampled_phi, sampled_s, hist_dens_estim, args=[bins, rg, 1./data_weights])
plt.imshow(parallel_sino_data.T, aspect="auto", extent=[0, np.pi, np.min(sampled_s).item(), np.max(sampled_s).item()])
if True:
...
from simuPET.reconstruction.functional_costs import weighted_squared_l2_loss, squared_l2_norm, \
                                                  sum_differentiable, TV_cost, powerit_PTAP

P = init_parallel_projector_CUPY_2D(spacing_of_s, number_of_s_samples, sampled_phi, vol_shp)
...
max_eigenvalue = powerit_PTAP(P, A_weight, niter=10); ws_loss.lip = max_eigenvalue

s_norm = squared_l2_norm(P.dim[0], alpha=10.)
...
result_l22 = GD2(data_plus_l2_cost, niter=40, min_max=[0,np.infty], force_nostrong=True, verbose_iter=5).reshape(vol_shp)[::-1,:]
result_l2 = GD(data_plus_l2_cost, niter=40, min_max=[0,np.infty], verbose=5).reshape(vol_shp)[::-1,:]
pl=plt.imshow(result_l2); plt.colorbar(pl); plt.show()

# TV
tv_cost = TV_cost(vol_shp, n_prox_iter=100, lam=750.01, proj=[0,np.infty], acc="None") #7000 for 100 bins, 1000 (-9.49db) or 1200 for 400
result_tv2 = APGD2(ws_loss, tv_cost, niter=40, verbose_iter=5, acc="BT").reshape(vol_shp)[::-1,:]
result_tv = APGD(ws_loss, tv_cost, niter=120, verbose=5, acc_method="BT").reshape(vol_shp)[::-1,:]

pl=plt.imshow(result_tv); plt.colorbar(pl); plt.show()
...
pl=plt.imshow(result_fbp); plt.colorbar(pl); plt.show()

result_bp = bp_reconstruction(proj_geom, vol_geom, parallel_sino_data.get())
result_bp = np.array(result_bp)
pl=plt.imshow(result_bp); plt.colorbar(pl); plt.show()

## COMPARISON
from simuPET.reconstruction.evaluate_reconstruction import compare_plots, rescale_and_compare, rmse, mse
fig = compare_plots([result_tv, result_fbp, result_bp], img, rescale=lambda x: x/np.sum(x), single_cb=False)
...
rescale_and_compare(result_tv, img, lambda x,y: np.max((x-y)[y>0])/np.max(y[y>0]))


## SCALING
avg_sens = np.sum(parallel_sino_weights)/parallel_sino_weights.size #average sensitivity
