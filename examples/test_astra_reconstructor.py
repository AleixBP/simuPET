...
parallel_sino_data = dens_resample(sino_data, sampled_phi, sampled_s, hist_dens_estim, args=[100, 1./data_weights], plot=True)
parallel_sino_data = np.load("hist_dense_weighted.npy")
parallel_sino_data = dens_resample(sino_data, sampled_phi, sampled_s, kde_dens_estim, args=[1e5, 0.025, 1./data_weights, False], plot=True)
...
plt.imshow(1./parallel_sino_weights.T, aspect="auto")

# FBP
fbp_rec = fbp_reconstruction(proj_geom, vol_geom, parallel_sino_data)
pl=plt.imshow(fbp_rec); plt.colorbar(pl); plt.show() #; plt.clim(0,10); plt.show()

# BP
bp_rec = bp_reconstruction(proj_geom, vol_geom, parallel_sino_data)
pl=plt.imshow(bp_rec); plt.colorbar(pl); plt.show()


## NON-WEIGHTED RESAMPLING
parallel_sino_data = dens_resample(sino_data, sampled_phi, sampled_s, hist_dens_estim, args=[100, None], plot=True)
parallel_sino_data = np.load("hist_dense_notweighted.npy")
parallel_sino_data = dens_resample(sino_data, sampled_phi, sampled_s, kde_dens_estim, args=[1e5, 0.025, None, False], plot=True)
...
for i in range(5):
    ...
    yuk = P.adjoint(P(yuk))
    plt.imshow(yuk.reshape(shp).get()); plt.show()
...
fp = P*rec_unif
plt.imshow(fp.reshape((number_of_phi_samples, number_of_s_samples)), aspect="auto"); plt.show()
bp2 = P.T*fp
plt.imshow(bp2.reshape(shp)); plt.show()
bp = P.T*parallel_sino_data
plt.imshow(bp.reshape(shp)); plt.show()

# More
radius=shp[0]/4.; norm_p=2
data = np.linalg.norm(np.mgrid[:shp[0], :shp[1]]-np.array([shp[0],shp[1]])[:,np.newaxis,np.newaxis]/2., ord=norm_p, axis=0)<radius
...
sino_data = np.linalg.norm(np.mgrid[:sino_shp[0], :sino_shp[1]]-np.array([sino_shp[0],sino_shp[1]])[:,np.newaxis,np.newaxis]/2., ord=norm_p, axis=0)<radius
...
# Normal BP
plt.imshow((P.T*unif).reshape(shp)); plt.show()

# FBP
rec_unif = fbp_reconstruction(proj_geom, vol_geom, unif)
plt.imshow(rec_unif); plt.show()


plt.plot(rec_unif[(420-radius):(420+radius),420]); plt.show()
