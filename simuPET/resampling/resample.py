...
def hist_dens_estim(sino_data, bins=100, rg=None, weights=None):
    ...
    hist_values, *sp = np.histogram2d(
        *sino_data.T, bins=bins, range=rg, weights=weights
    )
    sp = np.array(sp)
    sp_diff = sp[:, 1] - sp[:, 0]
    ...
    def rebinning(sino):
        ...
        indexes = (
            ((sino.T - sp_off) // sp_diff).astype(int) + np.array([0, 0], dtype=int)
        ).T
...
def kde_dens_estim(sino_data, resize=100000, bw=None, weights=None, plot=False):
    ...
    # Make periodic
    band = numpy.pi / 8
    ss = numpy.concatenate(
        (sino_data, sino_data[sino_data[:, 0] < band] + numpy.array([numpy.pi, 0])),
        axis=0,
    )
    ss = numpy.concatenate(
        (
            ss,
            sino_data[sino_data[:, 0] > (numpy.pi - band)]
            + numpy.array([-numpy.pi, 0]),
        ),
        axis=0,
    )
    if plot:
        plt.hist2d(*ss.T, bins=100)
        plt.show()
    if weights is None:
        ww = weights
    else:
        ww = numpy.concatenate((weights, weights[sino_data[:, 0] < band]), axis=0)
        ww = numpy.concatenate(
            (ww, weights[sino_data[:, 0] > (numpy.pi - band)]), axis=0
        )
        if plot:
            plt.hist2d(*ss.T, bins=100, weights=ww)
            plt.show()
    ...
    if isinstance(type(sino_data), numpy.ndarray):
        ...
        return lambda x: np.array(
            gaussian_kde(ss.T, bw_method=bw, weights=ww)(x.get())
        )  # bw_method=0.1,
...
def func_resample(sino_data, sampled_phi, sampled_s, plot=False):
    ...
    if plot:
        plt.hist2d(*unique_sino_data.T, bins=100, weights=sino_counts)
...
def dens_resample(
    sino_data, sampled_phi, sampled_s, dens_estim=hist_dens_estim, args=[], plot=False
):
    parallel_grid = np.meshgrid(sampled_phi, sampled_s)  # size (2, s, phi)
    xx, yy = parallel_grid
...
if __name__ == "__main__":
    ...
    pl = plt.imshow(w_parallel_sino_data.T, aspect="auto")
    plt.colorbar(pl)
    plt.show()

    # Weighted KDE
    w2 = xray_scanner(boxes, *sino_data.T)
    density2 = dens_estim(sino_data, resize=1e6, weights=1.0 / w2)
    parallel_sino_data2 = density2(parallel_pos).reshape(sino_shp[::-1]).T
    pl = plt.imshow(parallel_sino_data2.T, aspect="auto")
    plt.colorbar(pl)
    plt.show()
