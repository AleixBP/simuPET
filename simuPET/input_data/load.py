...
yo = np.fromfile('amyloidPET_FBP_TP0/LM/17598013_1946_20150604155500.000000.bf', dtype=np.uint16)
yo = np.fromfile('amyloidPET_FBP_TP0/LM/17598013_1946_20150604155500.000000.bf', dtype=np.uintc)
yo = np.fromfile('amyloidPET_FBP_TP0/LM/17598013_1946_20150604155500.000000.bf', dtype=np.uint32)
...
import nibabel
phantom = nibabel.loadsave.load('amyloidPET_FBP_TP0/LM/17598013_1946_20150604155500.000000.dcm')
...
if True:
    ma = nipet.mmrhist(datain, mMRpars, t0=3000, t1=3601, outpath=opth, store=True, use_stored=True)
...
plt.imshow((sim)[63,:,:]); plt.colorbar()
...
t0=3500
t1=3600
m = nipet.mmrhist(datain, mMRpars, t0=t0, t1=t1, outpath=opth, store=True, use_stored=True)
## Randoms
...
eim = nipet.mmrchain(datain, mMRpars, mu_h=None, mu_o=mu_o, histo=m, itr=1, outpath=opth)['im']
...
for k in [60, 64, 70, 89, 93, 98, 107]:
    print(k)
    plt.imshow(vol[k,:,:], cmap="magma"); plt.show()
...
plt.imshow(hh[93,:,:])
mMRpars
hh = nipet.frwd_prj(np.ones_like(vol), mMRpars) #, attenuation=True #with attenuation it does the integral
plt.imshow(hh[93,:,:])
N = nipet.mmrnorm.get_norm_sino(datain, mMRpars, m)
plt.imshow(N[93,:,:])
sho = 1./hh[93,:,:]
