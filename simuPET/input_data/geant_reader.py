...
def read_geant_file(geant_file, atol=.3, max_detections=None, auto_label=True, keep_opposites_only=True):
    ...
    if max_detections is None: max_detections = max_number(geant_file)

    hold = np.zeros((n_blocks, holding_buffer_size, n_values_per_line)) # 2 detections, 9 buffer
        ...
        for delimiter, event in iter.groupby(f, lambda line: line.startswith('#')):
            if not delimiter:
                try:
                    ...
                    for i, (block, detections) in enumerate(iter.groupby(event, lambda line: line[0])): # groupby: need to be sorted
                        if i>1: break # too many blocks involved (more than two)

                        detections_far = 0
                        for j, line in enumerate(detections):
                            hold[i, j] = [float(x) for x in line.split()[1:]] + [block_dict[line[0]]] # split line into floats

                            if not np.allclose(hold[i, 0][:3], hold[i, j][:3], atol=atol):
                                detections_far +=1

                        if j>0: # more than one detection in one block
                            if not detections_far: # they look close enough
                                n_close_events += 1
                                discard += handle_close_multidetections(i, j, hold) # mark and continue through i
                                # if nothing is done instead, then first detection is taken in hold[:,0,:-dont_keep]
    
                            else:
                                discard += handle_far_multidetections(i, j, hold) # mark as continue through i

                    if i>1 or discard>0 or (keep_only_opposite_blocks(hold) and keep_opposites_only): 
