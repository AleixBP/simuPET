...
class parallel_projector:  # LinearOperator (super)
    def __init__(self, proj_geom, vol_geom, lip=None, eigen_min=None):
        ...
        self.P = astra.OpTomo(
            astra.creators.create_projector("line", proj_geom, vol_geom)
        )  #'cuda'
        self.lip = np.infty if lip is None else lip
        self.eigen_min = 0 if eigen_min is None else eigen_min
        self.dim = [
            np.prod(np.asarray(astra.geom_size(vol_geom))),
            np.prod(np.asarray(astra.geom_size(proj_geom))),
        ]
...
def init_parallel_projector_2D(
    spacing_of_s, number_of_s_samples, sampled_phi, vol_shp, **kwargs
):
    proj_geom = astra.creators.create_proj_geom(
        "parallel", spacing_of_s, number_of_s_samples, sampled_phi
    )
...
class parallel_projector_CUPY:  # LinearOperator (super)
    ...
    def __init__(self, proj_geom, vol_geom, lip=None, eigen_min=None):  # cuda3d #line
        ...
        self.proj_id = astra.creators.create_projector(
            "cuda3d", proj_geom, vol_geom
        )  # cuda for 2d
        ...
        self.eigen_min = 0 if eigen_min is None else eigen_min
        self.dim = [
            int(np.prod(np.asarray(astra.geom_size(vol_geom)))),
            int(np.prod(np.asarray(astra.geom_size(proj_geom)))),
        ]
    ...
    def FPBP(self, x, out=None, mode="FP"):
        ...
        if mode == "FP":
            ...
            out = sino = (
                np.empty(self.sino_shp, dtype=np.float32) if out is None else out
            )
        ...
        vol_link = self.data_mod.GPULink(
            vol.data.ptr, *vol.shape[::-1], vol.strides[-2]
        )
        vid = self.data_mod.link("-vol", self.vol_geom, vol_link)

        sino_link = self.data_mod.GPULink(
            sino.data.ptr, *sino.shape[::-1], sino.strides[-2]
        )
...
def init_parallel_projector_CUPY_2D(
    spacing_of_s, number_of_s_samples, sampled_phi, vol_shp, **kwargs
):
    proj_geom = astra.creators.create_proj_geom(
        "parallel3d", 1.0, spacing_of_s, 1, number_of_s_samples, sampled_phi.get()
    )
...
def fbp_reconstruction(proj_geom, vol_geom, parallel_sino_data):
    ...
    alg_unif_cfg = astra.creators.astra_dict(
        "FBP_CUDA"
    )  # BP_CUDA, SIRT_CUDA, SART_CUDA, CGLS_CUDA
