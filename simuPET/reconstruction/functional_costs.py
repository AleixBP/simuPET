...
class squared_l2_norm:  # DifferentiableFunctional
    # 1/2 * alpha * ||x||^2
    def __init__(self, dim_in, alpha=2.0):
        self.alpha = alpha
        self.lip = alpha
...
class weighted_squared_l2_loss:  # DifferentiableLoss/DifferentiableFunctional
    ...
    def __init__(self, P, A=None, data=None, PTAP=None, PTAdata=None):
        ...
        self.data = data if data is not None else np.zeros(P.dim[0])
...
class l12_norm:
    ...
    def project(self, x):
        n = self.l2(x).clip(1.0)  # np.maximum(self.l2(x),1.)
        return (
            x.reshape(self.ndim, -1) / n
        ).ravel()  # x/(np.broadcast_to(n, (self.ndim, n.size)).flat) #(x.reshape(self.ndim,-1)/n).ravel()
...
class TV_cost:
    ...
    def __init__(
        ...
        from muPET.reconstruction.linear_operators import LinOpGrad

        self.Grad = LinOpGrad(
            shp, dtype=dtype
        )  # should reshape like .reshape(ndim,-1), but when? proj should take this into account.
        self.norm = l12_norm(shp)
        self.iter_func = lambda x: x if iter_func is None else iter_func
        self.acc = acc

        self.tv_ct = 8.0  # 12. for 3D?
    ...
    def prox(self, y, tau):
        ...
        ilip = 1.0 / (self.tv_ct * tau)
        x = np.zeros(self.size * self.ndim, dtype=y.dtype)
        x_prev = np.zeros_like(x)
        t_prev = 1.0

        for i in range(self.n_prox_iter):
            x_temp = x + ilip * self.Grad(self.proj(y - tau * self.Grad.adjoint(x)))
...
def powerit_PTAP(P, A, niter=10, stop=1.0):
...
if __name__ == "__main__":
    ...
    fd = pl.Gradient(img.shape)
    ndim = img.ndim
    x = img.ravel()
    fd(x)
    np.sum(frobenius(fd(x).reshape(ndim, -1), True))
    np.sum(np.linalg.norm(fd(x).reshape(ndim, -1), axis=0))
    nm = np.linalg.norm(fd(x).reshape(ndim, -1), axis=0)
    ah = np.maximum(nm, 1)
    fd(x).reshape(ndim, -1) / ah
    fd(x) / (np.broadcast_to(ah, (2, ah.size)).flat)

    plt.imshow(
        image[317:331, 555:570]
    )  # 35mm is 840 pixels, the dot is 10 pixels, i.e. 0.42mm (animal PET is around 1-2mm, human PET 4-6mm)
