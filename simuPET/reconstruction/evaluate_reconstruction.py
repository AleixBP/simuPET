...
def rmse(f, f_ref, normalize=False, db=False):
    ...
    return 5.0 * np.log10(ms) if db else np.sqrt(ms)
...
def psnr(f, f_ref):
    ...
    return 10.0 * np.log10(mse(f, f_ref) ** 2 / range)


def mutual_info(f, f_ref, bins=100):
    from scipy.stats import entropy  # cupy-compatible but not?

    hist_values, _ = np.histogramdd([f.ravel(), f_ref.ravel()], bins=bins, density=True)

    return (
        entropy(np.sum(hist_values, axis=0).get())
        + entropy(np.sum(hist_values, axis=1).get())
...
def compare_plots(f, f_ref, compare=None, rescale=None, single_cb=True):
    ...
    vmin = np.min(images).item()
    vmax = np.max(images).item()

    diffs = np.array(diffs)
    vmind, vmaxd = (
        (vmin, vmax) if single_cb else (np.min(diffs).item(), np.max(diffs).item())
    )
    ...
    if single_cb:
        fig.colorbar(
            im, ax=[axs[sz - 2], axs[sz - 1]], location="right", fraction=0.05
        )  # pad=0.04)
    else:
...
def fwhm_from_psf(psf, smooth=None, interp=False, plot=False):
    ...
    if not interp:
        ...
        cand = (
            mi - 1
            if (psf_x[mi] - max_val / 2.0) * (psf_x[mi + 1] - psf_x[mi]) > 0
            else mi + 1
        )
        if cand > mi:
            cand, mi = mi, cand
        hm = cand + (max_val / 2.0 - psf_x[cand]) / (psf_x[mi] - psf_x[cand])
        fwhm = 2.0 * np.abs(max_idx - hm)

    if plot:
        plt.plot(psf_x)
        plt.show()
        plt.plot(np.abs(psf_x - max_val / 2.0))
        plt.show()
