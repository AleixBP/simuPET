...
class updater:
    def __init__(
        ...
        if momentum_method == "SC" and all(
            hasattr(s, attr) for attr in ["lip", "eigen_min"]
        ):
        ...
        self.acc_dict = {
            "BT": lambda i, t_prev: 0.5 * (p + math.sqrt(q + self.r * t_prev**2)),
            "CD": lambda i, t_prev: (i + d) / d,
        }

        self.mom_dict = {
            "SC": lambda t, t_prev: self.s,
        }

        self.rest_dict = {
            "RESTART": lambda t, x_star, x_prev, y: (
                (t, x_star)
                if np.sum((y - x_star) * (x_star - x_prev)) < 0
                else (1 if restart_t else t, [x_prev, self.upr(xi * self.r)][0])
            ),
        }

        self.acc = self.acc_dict.get(acc_method, lambda i, t_prev: t_prev)
        self.mom = self.mom_dict.get(
            momentum_method, lambda t, t_prev: (t_prev - 1.0) / t
        )
        self.rest = self.rest_dict.get(
            restart_method, lambda t, x_star, x_prev, y: (t, x_star)
        )
...
def APGD(
    ...
    **kwargs
    ...
    # LIPS
    ilip = alpha / F.lip
    if update_ilip is True:
        update_ilip = lambda k, il, x: alpha / F.update_lip(x)
    if update_ilip is None:
        update_ilip = lambda k, il, x: ilip
    ...
    while True:
        ...
        if verbose and (itr % verbose == 0):
            costs.append(print_cost(F, G, x_star))
        if itr >= niter or converged(x_star, x_prev):
            break  # for/while not finished

        # ACCELERATE
        t = u.accelerate(itr, t_prev)
        if restart_enabled:
            t, x_star = u.maybe_restart(t, x_star, x_prev, y)
    ...
    # DONE
    if verbose:
        plt.plot(verbose * np.arange(len(costs)), costs)
        plt.show()
