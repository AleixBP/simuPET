...
def xray_square(theta, s):
    ...
    return (bspline(0.5 * s / s0, 0) + (s >= s0) * bspline((s - s0) / s1, 1)) / np.sin(
        theta
    )


def xray_rectangle(theta, s, a=[1.0, 1.0], xy=[0.0, 0.0], phi=0):
    theto = theta - phi
...
def geom_xray_scanner(
    theta, s, a=[1.0, 1.0], xy=[0.0, 0.0], lam=1, blocks=4, fov=[1.0, 1.0], extra=None
):
    hpi = np.pi / 2
    ...
    if extra is None:
        for i in range(blocks):
            ...
            result[condi] = (
                (
                    xray_rectangle(theta[condi], s[condi], a=a, xy=xy, phi=(phi + hpi))
                    == 0
                )
                * (
                    1
                    - np.exp(
                        -lam
                        * xray_rectangle(theta[condi], s[condi], a=a, xy=xy, phi=phi)
                    )
                )
                * (
                    1
                    - np.exp(
                        ...
                        * (
                            xray_rectangle(
                                theta[condi], s[condi], a=a, xy=xy, phi=(phi - hpi)
                            )
                            + xray_rectangle(
                                theta[condi], s[condi], a=a, xy=xy, phi=(phi - 2 * hpi)
                            )
                        )
                    )
                )
                ...
                * (
                    ...
                    - np.exp(
                        ...
                        * (
                            ...
                            + xray_rectangle(
                                theta[condi], s[condi], a=a, xy=xy, phi=(phi - 3 * hpi)
                            )
                        )
                    )
                )
    ...
    if blocks == 4:
        ...
        result[condi] = (
            ...
            - np.exp(
                -lam
                * (
                    xray_rectangle(theto, s[condi], a=a, xy=xy, phi=phi)
            ...
            - np.exp(
                -lam
                * (
                    xray_rectangle(theto, s[condi], a=a, xy=xy, phi=(phi + 2 * hpi))
...
def xray_scanner(boxes, theta, s):
    ...
    xy = [
        boxes.Rx - boxes.shift - boxes.layerLength / 2.0,
        boxes.Ry + boxes.shift + boxes.layerDepth / 2.0,
    ]
    extra = np.array(
        [
            [
                [boxes.layerLength - 2.0 * boxes.Rx, boxes.layerDepth],
                [2.0 * boxes.Rx, boxes.layerDepth],
            ],
            [
                [
                    -boxes.shift - boxes.layerLength / 2.0,
                    boxes.Ry + boxes.shift + boxes.layerDepth / 2.0,
                ],
                [-boxes.shift, boxes.Ry + boxes.shift + boxes.layerDepth / 2.0],
            ],
        ]
    )
    return geom_xray_scanner(
        theta, s, a=a, xy=xy, lam=boxes.lam, blocks=4, fov=boxes.fov, extra=extra
    )
...
def plot_intersect_in_scanner(boxes, ct, phi=0):
    ...
    if extra is None:
        ...
        condi = (
            (xray_rectangle(tt, ss, a=a, xy=xy, phi=phi) > 0)
            * (
                xray_rectangle(tt, ss, a=extra[0, 0], xy=extra[1, 0], phi=(phi - hpi))
                == 0
            )
            * (
                xray_rectangle(tt, ss, a=extra[0, 1], xy=extra[1, 1], phi=(phi + hpi))
                == 0
            )
        )
...
def compare_with_simulation(boxes, phi, s):
    plt.hist(
        phi[(s < 0.5) & (s > -0.5)], bins=100, density=True, stacked=True, alpha=0.5
    )
    thetas1000 = np.linspace(0, np.pi, 1000)
    xx = xray_scanner(boxes, thetas1000, np.zeros_like(thetas1000) + 0.0)
...
if __name__ == "__main__":
    ...
    # Plot square
    plt.contourf(tt, ss, xray_square(tt, ss))
    plt.show()
    ...
    plt.imshow(xray_rectangle(tt, ss, xy=[0.5, 0.0]))
    plt.imshow(xray_rectangle(tt, ss, xy=[0.0, 0.5]))
    plt.imshow(xray_rectangle(tt, ss, xy=[0.5, 0.5]))
    plt.imshow(xray_rectangle(tt, ss, a=[1.0, 1.2]))
    plt.imshow(xray_rectangle(tt, ss, xy=[0.5, 0.5], a=[1.0, 1.2], phi=np.pi / 2))
    plt.imshow(
        xray_rectangle(tt, ss, xy=[0.5, 0.5], a=[1.0, 1.2], phi=np.pi / 2)
        + xray_rectangle(tt, ss, xy=[0.5, 0.5], a=[1.0, 1.2], phi=0)
        + xray_rectangle(tt, ss, xy=[0.5, 0.5], a=[1.0, 1.2], phi=np.pi)
        + xray_rectangle(tt, ss, xy=[0.5, 0.5], a=[1.0, 1.2], phi=3 * np.pi / 2)
    )
    ...
    to, so = 0.78, 6.0
    pl = plt.imshow(all_cross3_scanner(boxes, tt, ss))
    plt.colorbar(pl)
    plt.scatter(
        (tt.shape[0] / np.max(tt)) * to,
        tt.shape[0] / 2.0 + (ss.shape[0] / (2 * np.max(ss))) * so,
    )
    plt.show()
    ...
    pl = plt.imshow(sens, extent=[0, np.pi, -lim, lim], aspect="auto")
    plt.colorbar(pl)
    plt.clim(0.02, 0.12)
    np.sum(
        sens
    )  # 7.817729939180599e+02 #without fov (e.g.big num boxes.fov=[100,100]) #with: 6.882479547482258e+02
    ...
    np.max(xx) / np.min(
        xx
    )  # 4.612851765655523e+00 #offset 0 (no fov does not really matter because central slice)
    ...
    pl = plt.imshow(xrs, extent=[0, np.pi, -lim, lim], aspect="auto")
    plt.colorbar(pl)
