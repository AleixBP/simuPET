...
def detector(x, pitch=0.1, parallax=0, radius=17, height=0.27):
    if parallax == 0:
        ...
        hp = float(
            (height * parallax + pitch * np.sqrt(radius**2 - parallax**2))
            / (2 * radius)
        )
...
pa = np.convolve(
    positron(xs), acollinearity(xs, gauss=True), mode="same"
)  # , mode="valid")
plt.plot(xs, pa / np.max(pa))
plt.xlim((-0.5, 0.5))
fwhm_pixels, _ = fw(pa)
...
def fwhm_vs_pitch(pitches, parallax=0, radius=17, height=0.27, step=0.001):
    ...
    for pitch in pitches:
        func = np.convolve(
            aux, detector(xs, pitch, parallax, radius, height), mode="same"
        )
...
plt.plot(
    pitches_dif[:-1],
    (fwhms_dif - np.min(fwhms_dif))
    * (nma - nmi)
    / (np.max(fwhms_dif) - np.min(fwhms_dif))
    + nmi,
)
...
plt.plot(pitches, fwhms)
plt.plot(
    (pitches_dif[-(-3 - ma + 1) :] + pitches_dif[: (-3 - ma + 1)]) / 2,
    (fwhms_dif - np.min(fwhms_dif))
    * (nma - nmi)
    / (np.max(fwhms_dif) - np.min(fwhms_dif))
    + nmi,
...
def fwhm_vs_radius(radiuses, pitch=0.1, parallax=0, height=0.27):
    ...
    for radius in radiuses:
        aux = np.convolve(
            positron(xs), detector(xs, pitch, parallax, radius, height), mode="same"
        )
...
def fwhm_vs_parallax(parallaxes, pitch=0.1, radius=17, height=0.27 / 2, step=0.001):
    ...
    for parallax in parallaxes:
        func = np.convolve(
            aux, detector(xs, pitch, parallax, radius=17, height=height), mode="same"
        )
        fwhm_pixels, _ = fw(func)
        fwhms.append(fwhm_pixels * step)
...
def fw(psf, ht=2.0, smooth=None, interp=False, plot=False):
    def slice_psf(psf):
    ...
    if psf.ndim == 1:
        ...
        max_idxs = np.unravel_index(np.argmax(psf), psf.shape)
        psf_x = (
            np.abs(psf[max_idxs[-2], :].copy())
            + np.abs(psf[max_idxs[-2] + 1, :].copy())
            + np.abs(psf[max_idxs[-2] - 1, :].copy())
        ) / 3.0
    ...
    if not interp:
        ...
        cand = (
            mi - 1
            if (psf_x[mi] - max_val / ht) * (psf_x[mi + 1] - psf_x[mi]) > 0
            else mi + 1
        )
        if cand > mi:
            cand, mi = mi, cand
    ...
    if plot:
        plt.plot(psf_x)
        plt.xlim(max_idx - 20, max_idx + 20)
        plt.show()
