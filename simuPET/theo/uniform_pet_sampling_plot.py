...
if mixed:
    num_points = 7#5
    solid = np.pi; solid_bool=True
else:
    num_points = 16
    solid = 2*np.pi; solid_bool=False
...
if precise: 
    ...
    xmt = np.linspace(-r-1-diff*(layers-1),r+1+diff*(layers-1), 7-2*(2-layers), endpoint=True)
...
ofb=r+off
num_points_mu = int(np.round((lengb+np.abs(ofb))/det_size)) if not tweak else 19 #15 # int((num_points*5*5-num_points_mu1*2*rat_doi)/rat_doi)
        ...
        for yl in y1:
            for lr in x2:
                for yr in y2:
                    ...
                    if circle_line_segment_intersection([0,0], r, [ll,yl], [lr,yr]) and not (np.abs(ll)>r and np.abs(lr)>r and ll*lr>0):
                        plt.plot([ll,lr], [yl,yr], 'k',alpha=alpha)
                    else:
                        pass    
